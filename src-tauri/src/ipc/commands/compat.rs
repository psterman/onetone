...
use crate::ipc::{
    start_trigger_compat_probe, start_trigger_verify_listen, stop_trigger_compat_probe,
    stop_trigger_verify_listen,
};
use crate::AppState;
...
#[tauri::command]
pub fn cmd_start_trigger_verify_listen(
    state: State<Arc<AppState>>,
    mapping_id: String,
) -> serde_json::Value {
    let id = mapping_id.trim();
    ...
    let ok = start_trigger_verify_listen(state.inner(), id);
    serde_json::json!({ "ok": ok, "reason": if ok { "" } else { "start_failed" } })
}

#[tauri::command]
pub fn cmd_stop_trigger_verify_listen(state: State<Arc<AppState>>) {
    crate::app_log::log_line(state.inner(), "verify", "cmd_stop_trigger_verify_listen");
    stop_trigger_verify_listen(state.inner());
}

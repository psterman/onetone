//! Cline file hooks — one script per event under `.cline/hooks` (+ Documents/Cline/Hooks).
//! Not settings.json. Marker: `--onetone-hook-id cline-activity-v1`.
...
pub const HOOK_ID: &str = "cline-activity-v1";
pub const PROBE_RESOURCE: &str = "scripts/cline-hook-probe.js";
...
fn build_cmd_body(probe_abs: &str, event: &str) -> String {
    format!(
        "@echo off\r\nnode \"{}\" --onetone-hook-id {} --source cline --event {}\r\n",
        probe_abs.replace('\\', "/"),
        HOOK_ID,
        event
    )
...
fn file_is_ours(path: &Path) -> bool {
    fs::read_to_string(path)
        .map(|s| s.contains(HOOK_ID) && s.contains("cline-hook-probe"))
        .unwrap_or(false)
}
...
pub fn setup_status() -> Result<ShellHookSetupStatus, String> {
    ...
    let primary = dirs
        .first()
        .cloned()
        .unwrap_or_else(|| home_dir().join(".cline").join("hooks"));
...
mod tests {
    ...
    fn cmd_body_has_marker_and_event() {
        let body = build_cmd_body("C:/repo/scripts/cline-hook-probe.js", "TaskComplete");
        assert!(body.contains(HOOK_ID));
        assert!(body.contains("--event TaskComplete"));
        assert!(body.contains("--source cline"));
    }
}

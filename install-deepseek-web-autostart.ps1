...
# One-shot installer:
#   1. Drops Start-DeepSeek-Web.vbs into shell:startup  (runs at every logon)
#   2. Optionally also registers a Task Scheduler entry "AtStartup" (no logon required)
#
# Usage (PowerShell, no admin needed for the Startup-folder install):
#   .\install-deepseek-web-autostart.ps1
...
if ($AlsoCreateTask) {
    $arg = "`"$vbs`""
    # /SC ONSTART runs at boot without needing an interactive user.
    # /RL HIGHEST asks for elevation so it survives UAC; /F overwrites.

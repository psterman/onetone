...
assert.ok(Shared.channelVizHtml('all',m).includes('habit-ws-key-card'),'overview viz renders stream deck key cards');
assert.ok(Shared.channelVizHtml('all',m).includes('habit-ws-key-card-desc'),'key card has summary desc');

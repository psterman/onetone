...
assert.match(hub, /adoptForegroundIdentity/);
assert.match(hub, /cmd_habit_foreground_app/);
assert.match(hub, /function adoptForegroundIdentity/);
const acceptFg=hub.slice(hub.indexOf('function acceptForegroundRecommend'), hub.indexOf('function openExistingFgScenario'));
assert.match(acceptFg, /openCreatedScenarioChannel\(m\)/);
assert.doesNotMatch(acceptFg, /restoreBaselineSelection/);

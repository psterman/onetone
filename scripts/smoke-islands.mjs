...
if (existsSync(bundle)) {
  ...
  check('P12 习惯列表岛已移除（无 __otHabitHubListSync）', !code.includes('__otHabitHubListSync'));
  check('P12 习惯列表岛挂载入口已移除', !code.includes('__otMountHabitHubListIsland'));
...
const habitSharedJs = readFileSync(resolve(root, 'src/js/features/mapping/habit-shared.js'), 'utf8');
...
check('P12 scheduleHubPaint 委托 Workspace', habitHubJs.includes('paintWorkspace') && habitHubJs.includes('scheduleHubPaint'));
check('P12 Workspace sidebar 批量管理', habitSharedJs.includes('data-habit-batch-toggle') && habitSharedJs.includes('data-habit-select'));
...
check('P12 settings-drawer 不再挂载 list 岛', !settingsDrawerJs.includes('__otMountHabitHubListIsland'));

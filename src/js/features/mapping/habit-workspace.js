(function(global){
  ...
  function quickDetail(m,channel,itemId){
    ...
    if(itemId==='softpad-status'){
      var lights=['codexStatusLightsEnabled','claudeStatusLightsEnabled','cursorStatusLightsEnabled','workbuddyStatusLightsEnabled','traeStatusLightsEnabled','qoderStatusLightsEnabled'].filter(function(k){ return !!pad[k]; }).length;
  ...
  function modeHtml(mode){
    return '<div class="habit-ws-modebar"><div class="habit-ws-modes" role="tablist" aria-label="Habit view"><button type="button" role="tab" aria-selected="'+(mode==='quick'?'true':'false')+'" class="'+(mode==='quick'?'is-selected':'')+'" data-habit-mode="quick">'+esc(c('quick'))+' <em>'+esc(c('recommended'))+'</em></button><button type="button" role="tab" aria-selected="'+(mode==='programmer'?'true':'false')+'" class="'+(mode==='programmer'?'is-selected':'')+'" data-habit-mode="programmer">'+esc(c('programmer'))+'</button></div>'+(mode==='programmer'?'<button type="button" class="habit-ws-back-quick" data-habit-mode="quick">← '+esc(c('backQuick'))+'</button>':'')+'</div>';
  }
  function introHtml(){

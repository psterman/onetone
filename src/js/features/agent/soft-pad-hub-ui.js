(function (global) {
  ...
  /** True only after the user picks a Hub app. Implicit 'codex' must not hide Cursor. */
  var scopePickedByUser = false;
  ...
  var VALID_SOFT_PAD_PAD_MODES = { keys: 1, show: 1, skin: 1 };
  ...
  /** Legacy tabs → 改按钮 / 何时显示 / 皮肤. */
  function normalizeSoftPadPadMode(mode) {
    mode = String(mode || '').trim();
    if (mode === 'style' || mode === 'appear' || mode === 'runtime' ||
        mode === 'display' || mode === 'purpose') mode = 'show';
    if (mode === 'look' || mode === 'presentation' || mode === 'agent' ||
        mode === 'lights' || mode === 'mini') mode = 'skin';
    return mode;
  }

  function isSoftPadLookMode(mode) {
    mode = normalizeSoftPadPadMode(mode == null ? softPadPadMode : mode);
    return mode === 'show' || mode === 'skin';
  }
  ...
  /** Default Hub tab — prefer live FG agent, then any enabled pad; never invent Codex. */
  function pickHubDefaultScopeId(entries) {
    entries = entries || listSoftPadSchemes();
    var i;
    var fgKind = kindForAppId(getFreshForegroundAppId());
    if (fgKind) {
      for (i = 0; i < entries.length; i++) {
        if (entries[i].kind === fgKind && hasMapping(entries[i])) return fgKind;
      }
    }
    for (i = 0; i < entries.length; i++) {
      if (entries[i].padEnabled && entries[i].kind !== SOFT_PAD_UNIVERSAL_KIND) {
        return entries[i].kind;
      }
    }
    for (i = 0; i < entries.length; i++) {
      if (entries[i].kind !== SOFT_PAD_UNIVERSAL_KIND && hasMapping(entries[i])) {
        return entries[i].kind;
      }
    }
   ...
   * Auto: foreign app → that app's page or 通用; else waiting → foreground agent → fallback.
   * Opening Cursor/Claude/… beats another agent's waiting. User pin removed.
  ...
  function resolvePrimaryLaneResult(entries, ctx) {
    ...
    if (ctx.foreignHost) {
      var fgApp = String(ctx.foregroundAppId || '').trim();
      var a;
      if (fgApp && !kindForAppId(fgApp)) {
        for (a = 0; a < entries.length; a++) {
          if (entries[a] && entries[a].appId === fgApp && entries[a].padEnabled) {
            return { entry: entries[a], reason: LANE_REASONS.foreground };
          }
        }
      }
      for (a = 0; a < entries.length; a++) {
        if (entries[a] && entries[a].kind === SOFT_PAD_UNIVERSAL_KIND) {
          return { entry: entries[a], reason: LANE_REASONS.foreground };
        }
      }
      return { entry: null, reason: LANE_REASONS.none };
    }
  ...
  /** Chrome and other non-agent windows. OneTone itself and tray shell stay put. */
  function foreignHostFromLane(fgAppId, fgFresh) {
    var id = null;
    try {
      var ui = global.OneToneState && global.OneToneState.ui;
      id = ui && ui.habitHubFgIdentity;
    } catch (_) {}
    if (id) {
      var exe = String(id.exeName || id.exe_name || '').toLowerCase();
      var path = String(id.fullPath || id.full_path || '').toLowerCase();
      var self = exe.indexOf('onetone') >= 0 || path.indexOf('onetone') >= 0 || path.indexOf('voice-pilot') >= 0;
      if (!self && exe && !isTrayFgNoise(exe)) {
        var preset = String(id.matchedPresetAppId || id.matched_preset_app_id || id.appId || '').trim();
        if (!kindForAppId(preset)) return true;
      }
    }
    return !!(fgFresh && fgAppId && !kindForAppId(fgAppId));
  }
  ...
  function laneContextFromRuntime() {
    ...
    return {
      ...
      foreignHost: foreignHostFromLane(fgFresh ? fg : '', fgFresh),
  ...
  function isAgentInstalledForKind(kind) {
    var inv = hubInventoryByKind[String(kind || '')] || null;
    return !!(inv && (inv.confidence === 'high' || inv.running));
  }

  function isAgentInstalledForAppId(appId) {
    return isAgentInstalledForKind(kindForAppId(appId));
  }

  function isVisibleSoftPadScope(scope, activeId) {
    if (!scope) return false;
    if (String(scope.id) === SOFT_PAD_UNIVERSAL_KIND) return true;
    if (String(scope.id) === String(activeId || selectedScopeId || '')) return true;
    if (scope.installed || scope.connected || scope.padEnabled) return true;
    var entry = scope.entry || entryForScopeKind(scope.id);
    if (entry && hasMapping(entry)) return true;
    if (entry && entry.mapping && entry.mapping.codexMicroPad) return true;
    return false;
  }

  function isScopeVisibleForMenu(kindOrAppId) {
    var kind = String(kindOrAppId || '');
    if (kind.indexOf('-') >= 0) kind = kindForAppId(kind) || kind;
    if (kind === SOFT_PAD_UNIVERSAL_KIND) return true;
    if (kind === String(selectedScopeId || '')) return true;
    if (isAgentInstalledForKind(kind)) return true;
    var entry = entryForScopeKind(kind);
    if (entry && hasMapping(entry)) return true;
    if (entry && entry.padEnabled) return true;
    if (entry && entry.mapping && entry.mapping.codexMicroPad) return true;
    return false;
  }
  ...
  function listAppScopes() {
    ...
    var activeId = String(selectedScopeId || '');
    scopes = scopes.filter(function (s) { return isVisibleSoftPadScope(s, activeId); });
  ...
  function els() {
    return {
      ...
      pageRail: document.getElementById('softPadPageRail'),
      channelNav: document.getElementById('softPadChannelNav'),
      previewHint: document.getElementById('softPadPreviewHint'),
      rightCol: document.getElementById('softPadRightCol'),
      channelBridge: document.getElementById('softPadChannelBridge'),
      channelBridgeTitle: document.getElementById('softPadChannelBridgeTitle'),
      channelBridgeBody: document.getElementById('softPadChannelBridgeBody'),
      channelBridgeGo: document.getElementById('softPadChannelBridgeGo'),
      workChannelPill: document.getElementById('softPadWorkChannelPill')
    };
  }

  function ensureSoftPadThreeColLayout() {
    var body = document.getElementById('softPadPageBody');
    if (body && body.classList) body.classList.add('soft-pad-page-body--three-col');
  }

  function softPadChannelLabel(ch) {
    var map = {
    ...
    var row = map[ch] || map.softPad;
    return t(row[0], row[1]);
  }

  function syncSoftPadRailChrome() {
    var ch = softPadRailChannel === 'softPad' ? 'softPad' : softPadRailChannel;
    var nav = document.getElementById('softPadChannelNav');
    if (nav) {
      var items = nav.querySelectorAll('[data-soft-pad-channel]');
      for (var i = 0; i < items.length; i++) {
        var btn = items[i];
        var channel = btn.getAttribute('data-soft-pad-channel') || '';
        var on = channel === ch;
        btn.classList.toggle('is-active', on);
        btn.classList.remove('is-keys-page');
        btn.setAttribute('aria-selected', on ? 'true' : 'false');
      }
    }
    var pill = document.getElementById('softPadWorkChannelPill');
  ...
  function paintSoftPadRailChannelWorkbench(ch) {
    ch = String(ch || softPadRailChannel || 'softPad').trim();
    var entry = resolveSoftPadEntry();
    if (!entry || !hasMapping(entry)) return;
    ...
    if (typeof Pad.renderSoftPadChannelWorkbench === 'function') {
      var keepKey = '';
      try {
        if (typeof Pad.getSoftPadLayoutFocusKeyId === 'function') {
          keepKey = String(Pad.getSoftPadLayoutFocusKeyId() || '').trim();
        }
      } catch (_) {}
      try {
        Pad.renderSoftPadChannelWorkbench(paintHost, entry.mapping, ch, {
          focusKeyId: keepKey
        });
      } catch (_) {}
    }
  ...
  function setSoftPadRailChannel(ch, opts) {
    opts = opts || {};
    ch = String(ch || 'softPad').trim();
    if (!/^(ime|key|voice|cursor|softPad|camera)$/.test(ch)) ch = 'softPad';
    softPadRailChannel = ch;
    syncSoftPadRailChrome();
    if (opts.fromUser || opts.forcePaint) {
      paintSoftPadRailChannelWorkbench(ch);
    }
  }

  function openSoftPadChannelInKeys(channel) {
    channel = String(channel || '').trim() || 'ime';
    try {
      var drawer = global.OneToneSettingsDrawer;
      if (drawer && typeof drawer.open === 'function') drawer.open({ panel: 'keys' });
      else if (drawer && typeof drawer.setPanel === 'function') drawer.setPanel('keys');
    } catch (_) {}
    setTimeout(function () {
      try {
        var picker = global.OneToneKeysChannelCommandPicker;
        if (picker && typeof picker.setActiveTab === 'function') picker.setActiveTab(channel);
      } catch (_) {}
    }, 80);
  }
  ...
  function refreshSoftPadSceneKeys(entry) {
    ...
    var e = els();
    ensureSoftPadThreeColLayout();
    var Panel = global.OneToneKeysSceneActionsPanel;
    var face = softPadFace === 'pad' && normalizeSoftPadPadMode(softPadPadMode) !== 'skin';
    ...
    if (!show) {
      ...
      // three-col keeps right column; has-scene-panel only marks habits visibility for CSS that still looks for it
  ...
  function isAgentWorkbenchMode() {
    // Top tabs are keys/show/skin; agent workbench only via retired agent face.
    return softPadFace === 'agent';
  }

  /** One-shot map for openSubpage / forceView compat. */
  function legacyViewToRoute(view) {
    view = String(view || '');
    if (view === 'advanced') view = 'skin';
    if (view === 'agent' || view === 'lights' || view === 'mini') return { face: 'pad', mode: 'skin' };
    if (view === 'timeline') return { face: 'timeline', mode: null };
    if (view === 'purpose' || view === 'runtime' || view === 'appear' || view === 'display') {
      return { face: 'pad', mode: 'show' };
    }
    if (view === 'presentation' || view === 'look') {
      return { face: 'pad', mode: 'skin' };
    }
    if (view === 'style') return { face: 'pad', mode: 'show' };
  ...
  function previewHostForFace(face) {
    ...
    // Always use pad preview host for merged lights/mini (agent host retired).
  ...
  function panelPaintBody(panel) {
    ...
    // Agent workbench paints into pad subpage body after face merge.
    if (panel === 'agent') return e.subBody || e.agentBody;
  ...
  function syncPadTabs() {
    var e = els();
    if (!e.padTabs) return;
    // 改按钮 / 何时显示 / 皮肤 — mid-ability card tablist (same card as page content).
    var show = softPadFace === 'pad' && softPadAgentReady(resolveSoftPadEntry());
    e.padTabs.hidden = !show;
    e.padTabs.setAttribute('aria-hidden', show ? 'false' : 'true');
    e.padTabs.querySelectorAll('[data-pad-mode]').forEach(function (btn) {
      var mode = btn.getAttribute('data-pad-mode') || '';
      btn.hidden = false;
      btn.removeAttribute('aria-hidden');
      var on = show && mode === softPadPadMode;
      btn.classList.toggle('is-active', on);
      btn.setAttribute('aria-selected', on ? 'true' : 'false');
    });
  ...
  /** Keys mode: right bind column + float dock. Show/skin: mid-ability panels. */
  function softPadKeysWorkbenchOpen(entry) {
    return softPadAgentReady(entry) &&
      softPadFace === 'pad' &&
      normalizeSoftPadPadMode(softPadPadMode) === 'keys';
  }

  function isSoftPadKeysWorkbenchOpen() {
    return softPadKeysWorkbenchOpen(resolveSoftPadEntry());
  }

  function ensureSoftPadFloatDock(entry) {
    if (!softPadKeysWorkbenchOpen(entry) || !hasMapping(entry)) return;
    var Pad = global.OneToneCodexMicroPadUi;
    if (!Pad || typeof Pad.paintSoftPadFloatDock !== 'function') return;
    // Keys workbench owns dock; force keys so show/skin visits don't leave float tab sticky.
    try { Pad.paintSoftPadFloatDock(entry.mapping, { floatTab: 'keys' }); } catch (_) {}
  }
      ...
      .then(function (snap) {
        ...
        if (!opts.silent && isSoftPadPageVisible()) {
          ...
          setTimeout(function () {
            ...
            try {
              var PadStats = global.OneToneCodexMicroPadUi;
              var statsEntry = findEntry(getSelectedMappingId());
              if (PadStats && typeof PadStats.paintSoftPadPreviewStats === 'function' &&
                  statsEntry && statsEntry.mapping) {
                PadStats.paintSoftPadPreviewStats(statsEntry.mapping);
              }
            } catch (_) {}
          }, 0);
        }
  ...
  var foregroundFollowTimer = 0;
  var foregroundFollowToken = '';

  function ensureForegroundFollow() {
    if (foregroundFollowTimer) return;
    followForegroundOnce();
   ...
   * Universal Soft Pad mapping is the open-page default — never sticky over FG agent.
   */
  function resolveSoftPadEntry() {
    var byId = findEntry(getSelectedMappingId());
    // Concrete agent Soft Pad only — universal baseline is not a loaded agent scene.
    if (hasMapping(byId) && byId.kind !== SOFT_PAD_UNIVERSAL_KIND) return byId;
    // Restart used to keep the module default scope "codex" and open Codex prepare
    // even when Cursor was foreground and already had a Soft Pad scene.
    if (!scopePickedByUser) {
      var fgKind = kindForAppId(getFreshForegroundAppId());
      var fgEntry = fgKind ? entryForScopeKind(fgKind) : null;
      if (hasMapping(fgEntry)) return fgEntry;
      var schemes = listSoftPadSchemes();
      var i;
      var hit = null;
      for (i = 0; i < schemes.length; i++) {
        if (!schemes[i] || schemes[i].kind === SOFT_PAD_UNIVERSAL_KIND) continue;
        if (!hasMapping(schemes[i])) continue;
        if (schemes[i].padEnabled) return schemes[i];
        if (!hit) hit = schemes[i];
      }
      if (hit) return hit;
    }
    var scope = findScope(selectedScopeId || 'codex');
    if (scope && hasMapping(scope.entry) && scope.entry.kind !== SOFT_PAD_UNIVERSAL_KIND) {
      return scope.entry;
    }
    // Current app chip still needs「准备」— don't steal another app's pad for preview.
    if (scopePickedByUser && scope && scope.canPrepare) return scope.entry || null;
  ...
  function adoptSoftPadSelection(entry) {
    ...
    syncHabitEditFromSoftPadEntry(entry);
    // Top chrome ("正在编辑 / 已与前台一致") paints before Soft Pad adopt — refresh it.
    try {
      var Banner = global.OneToneHabitChannelEditBanner;
      if (Banner && Banner.renderAll) Banner.renderAll();
    } catch (_) {}
  ...
  function syncHabitEditFromSoftPadEntry(entry) {
    var uiState = global.OneToneState && global.OneToneState.ui;
    if (!uiState) return;
    var diffMod = global.OneToneHabitOverrideDiff;
    var m = entry && entry.mapping;
    if (m && diffMod && diffMod.isAppScenarioMapping && diffMod.isAppScenarioMapping(m)) {
      uiState.habitScenarioReturnId = m.id;
      var panel = String(uiState.settingsPanel || 'softPad');
      uiState.habitScenarioReturnPanel = panel === 'voiceWake' ? 'voiceWake' : panel;
      uiState.cameraEditMode = 'appScenario';
      if (panel === 'voiceWake') uiState.voiceEditSchemeId = m.id;
    } else if (entry && entry.kind === SOFT_PAD_UNIVERSAL_KIND) {
      uiState.habitScenarioReturnId = null;
      uiState.habitScenarioReturnPanel = null;
      uiState.habitHubEditReturn = false;
      uiState.cameraEditMode = 'global';
      uiState.voiceEditSchemeId = '__global__';
      var baseline = findBaselineMapping();
      if (baseline && baseline.id && global.OneToneState && global.OneToneState.state) {
        global.OneToneState.state.selectedMappingId = baseline.id;
      }
    }
  }

  function statusBarEntry() {
    return resolveSoftPadEntry();
  }

  function canToggleSoftPadStatus(entry) {
    if (!entry) return false;
    applyRuntimeFlagsToEntry(entry);
    return !!(hasMapping(entry) || entry.canPrepare || entry.canEnable);
  }
  ...
  function syncAgentPreviewUsageStrip() {
    if (!isAgentWorkbenchMode()) return;
    try {
      var Pad = global.OneToneCodexMicroPadUi;
  ...
  function buildBindAppProps() {
    ...
    return {
      activeScope: active,
      bindLabel: t('settingsScopeSwitchLbl', '切换 · {name}').replace('{name}', appTitleFor(active)),
  ...
  function buildStatusProps(entry) {
    ...
    return mergeUsageIntoStatusProps({
      ...
      canToggle: canToggleSoftPadStatus(entry),
      activeScope: bind.activeScope,
      bindLabel: bind.bindLabel,
      scopes: bind.scopes
  ...
  function updateStatusBar(entry) {
    // P10: React island owns the slim status bar when mounted.
    // Preview-hint chrome (meta / bind / actions) stays in static DOM — always paint those.
    var e = els();
    var props = buildStatusProps(entry);
    if (global.__otSoftPadStatusMounted) {
      if (global.__otSoftPadStatusSync) global.__otSoftPadStatusSync(props);
    } else {
  ...
  function syncBindAppControl(props) {
    ...
    if (e.bindAppLbl) {
      e.bindAppLbl.textContent = props.bindLabel || t('settingsScopeSwitchLbl', '切换 · {name}').replace('{name}', '—');
    }
  ...
  function bindMenuPillsHtml(entry) {
    if (!entry) return '';
    var Connect = global.OneToneSoftPadConnect;
    var attn = global.OneToneSoftPadConnectAttn;
    var st = global.OneToneState && global.OneToneState.state;
    var phase = Connect && Connect.phaseOf ? Connect.phaseOf(entry.kind, st, attn) : 'unknown';
    var padPill = agentDirectoryPadPill(entry);
    var lightsPill = agentDirectoryLightsPill(entry);
    var connCls = agentDirectoryConnectPillClass(phase);
    var connText = agentDirectoryConnectPillText(phase);
    return (
      '<span class="soft-pad-agent-directory__pills">' +
      '<span class="soft-pad-agent-pill ' + esc(padPill.cls) + '">' + esc(padPill.text) + '</span>' +
      '<span class="soft-pad-agent-pill ' + esc(lightsPill.cls) + '">' + esc(lightsPill.text) + '</span>' +
      '<span class="soft-pad-agent-pill ' + esc(connCls) + '">' + esc(connText) + '</span>' +
      '</span>'
    );
  }

  function entryForScopeKind(scopeId) {
    var entries = listAsideEntries();
    var i;
    for (i = 0; i < entries.length; i++) {
      if (entries[i] && entries[i].kind === scopeId) return entries[i];
    }
    return null;
  }

  function softPadScopeSubline(scopeId) {
    if (String(scopeId || '') === SOFT_PAD_UNIVERSAL_KIND) {
      return t('settingsScopeSubGlobal', '所有应用默认');
    }
    return t('softPadScopeAgentSub', 'Agent 应用场景');
  }

  function softPadScopeSwitchLabel() {
    return t('settingsScopeSwitchLbl', '切换 · {name}').replace('{name}', appTitleFor(selectedScopeId || 'codex'));
  }

  function renderSoftPadScopeMenuItems(activeId) {
    activeId = String(activeId || selectedScopeId || 'codex');
    var Scope = global.OneToneSettingsScopeSwitch;
    if (!Scope || !Scope.renderScopeMenuRow) return '';
    var props = buildBindAppProps();
    return (props.scopes || []).map(function (scope) {
      return Scope.renderScopeMenuRow({
        id: scope.id,
        panel: 'softPad',
        name: scope.title,
        active: String(scope.id) === activeId,
        iconHtml: iconHtml(scope.id, 'settings-scope-switch__icon-img')
      });
    }).join('');
  }

  function renderBindAppMenu(props) {
    if (global.__otSoftPadStatusMounted) return;
    var e = els();
    ...
    e.bindAppMenu.innerHTML = scopes.map(function (scope) {
      var entry = null;
      for (var i = 0; i < entries.length; i++) {
        if (entries[i] && entries[i].kind === scope.id) {
          entry = entries[i];
          break;
        }
      }
      return (
        ...
        '<span class="soft-pad-agent-directory__name">' + esc(scope.title) + '</span>' +
        bindMenuPillsHtml(entry) +
        '</button>'
      );
    }).join('');
  }
  ...
  function handleStatusAction(action) {
    ...
    if (action === 'open-tray') {
      if (global.OneToneSettingsDrawer && global.OneToneSettingsDrawer.open) {
        global.OneToneSettingsDrawer.open({ panel: 'tray' });
      }
      return;
    }
  ...
  function clearSubpage() {
    ...
    if (e.subHost) {
      ...
      // Keep hidden — keys mode must not flash mid-ability; chrome sync re-opens for 样子/更多.
      e.subHost.hidden = true;
      e.subHost.setAttribute('aria-hidden', 'true');
    }
    if (e.subTitle) {
  ...
  function setDetailOpen(open) {
    ...
    if (e.subHost) {
      ...
      // syncSoftPadNeedAgentChrome owns subHost.hidden for 改按钮 vs 样子/更多.
    }
  ...
  function emptyCreateCtaHtml() {
    var scopes = listAppScopes().filter(function (s) {
      return s && s.kind && s.kind !== SOFT_PAD_UNIVERSAL_KIND && isHubSoftPadKind(s.kind);
    });
    var chips = scopes.map(function (s) {
      var ready = !!(
        (s.mapping && s.mapping.codexMicroPad) ||
        (s.entry && s.entry.mapping && s.entry.mapping.codexMicroPad)
      );
      return (
        '<button type="button" class="codex-micro-pad__btn' +
        (ready ? ' codex-micro-pad__btn--primary' : '') +
        '" data-soft-pad-load-scope="' +
        esc(s.kind) +
        '">' +
        esc(s.title || appTitleFor(s.kind)) +
        (ready ? '' : ' · ' + esc(t('softPadHubPrepareShort', '准备'))) +
        '</button>'
      );
    }).join('');
    if (!chips) {
      chips =
        ...
        esc(t('softPadEmptyCreateCodex', '创建 Codex 应用场景')) +
        '</button>' +
        '<button type="button" class="codex-micro-pad__btn" data-soft-pad-create-kind="claude">' +
        esc(t('softPadEmptyCreateClaude', '创建 Claude 应用场景')) +
        '</button>';
    }
    return (
      '<p class="soft-pad-empty__title">' +
      esc(t('softPadNeedAgentTitle', '先加载一个 Agent')) +
      '</p>' +
      '<p class="soft-pad-empty__desc">' +
      esc(
        t(
          'softPadNeedAgentDesc',
          '屏幕按钮要绑在具体 Agent 上（Cursor / Codex / Claude…）。选一个加载后，再显示键盘与右侧动作栏。'
        )
      ) +
      '</p>' +
  ...
  function bindEmptyCreateCtas(host) {
    ...
    host.querySelectorAll('[data-soft-pad-load-scope]').forEach(function (btn) {
      if (btn.dataset.softPadLoadBound === '1') return;
      btn.dataset.softPadLoadBound = '1';
      btn.addEventListener('click', function () {
        var kind = btn.getAttribute('data-soft-pad-load-scope') || '';
        if (!kind) return;
  ...
  function buildSoftPadEmptyIdleModel() {
    ...
    if (mode === 'empty') {
      emptyHidden = false;
      emptyTitle = t('softPadNeedAgentTitle', '先加载一个 Agent');
      emptyDesc = t(
        'softPadNeedAgentDesc',
        '屏幕按钮要绑在具体 Agent 上（Cursor / Codex / Claude…）。选一个加载后，再显示键盘与右侧动作栏。'
      );
  ...
  function renderEmptyMain() {
    ...
    syncSoftPadNeedAgentChrome(null);
  }
  ...
  function hideEmpty() {
    ...
    emptyPrepareCtx = null;
    applySoftPadEmptyIdleHost(buildSoftPadEmptyIdleModel());
    syncSoftPadNeedAgentChrome(resolveSoftPadEntry());
  }

  /**
   * Concrete Agent Soft Pad ready (not universal / placeholder).
   * Right habits column + mid command list only show when this is true.
   */
  function softPadAgentReady(entry) {
    entry = entry || resolveSoftPadEntry();
    if (!hasMapping(entry)) return false;
    var kind = String(entry.kind || selectedScopeId || '').trim();
    if (!kind || kind === SOFT_PAD_UNIVERSAL_KIND) return false;
    if (!isHubSoftPadKind(kind)) return false;
    return !!(entry.mapping && entry.mapping.codexMicroPad);
  }

  /**
   * No Agent loaded → prompt; hide right mid card.
   * With Agent → mid-ability card always on (tabs + keys/style/agent pages).
   */
  function syncSoftPadNeedAgentChrome(entry) {
    var e = els();
    ...
    if (e.pageBody) {
      e.pageBody.classList.toggle('is-need-agent', need);
    }
    if (e.rightCol) {
      if (need) {
        e.rightCol.setAttribute('hidden', '');
        e.rightCol.setAttribute('aria-hidden', 'true');
      } else {
        e.rightCol.removeAttribute('hidden');
        e.rightCol.setAttribute('aria-hidden', 'false');
      }
    }
    if (e.stage) {
      if (need && (emptySurfaceMode === 'empty' || emptySurfaceMode === 'prepare')) {
        e.stage.setAttribute('hidden', '');
        e.stage.setAttribute('aria-hidden', 'true');
      } else {
        e.stage.removeAttribute('hidden');
        e.stage.setAttribute('aria-hidden', 'false');
      }
    }
    // Mid card hosts tabs + page; stay open whenever an Agent Soft Pad is ready.
    if (e.subHost) {
      var cardOpen = ready && softPadFace === 'pad';
      e.subHost.hidden = !cardOpen;
      e.subHost.setAttribute('aria-hidden', cardOpen ? 'false' : 'true');
      if (cardOpen) {
        e.subHost.classList.add('is-open');
        e.subHost.removeAttribute('hidden');
      }
    }
    var fnSwap = document.getElementById('softPadFnSwapHost');
    if (fnSwap) {
      fnSwap.hidden = !keysWb;
      fnSwap.setAttribute('aria-hidden', keysWb ? 'false' : 'true');
    }
    if (e.detailPanel) {
    ...
    syncPadTabs();
    var midData = document.getElementById('softPadMidData');
    if (midData) {
      // 何时显示 / 皮肤：用量卡会挤掉左侧 Soft Pad 呼应，先收起。
      var hideData = ready && softPadFace === 'pad' && isSoftPadLookMode();
      midData.hidden = hideData;
      midData.setAttribute('aria-hidden', hideData ? 'true' : 'false');
    }
    if (e.sceneKeys && need) {
      e.sceneKeys.setAttribute('hidden', '');
      e.sceneKeys.setAttribute('aria-hidden', 'true');
    }
    if (e.pageBody && need) e.pageBody.classList.remove('has-scene-panel');
  }
  ...
  function subpageTitle(view) {
    ...
    if (view === 'runtime' || view === 'style' || view === 'show') return t('softPadPadTabShow', '何时显示');
    if (view === 'agent' || view === 'skin' || view === 'presentation') return t('softPadPadTabSkin', '皮肤');
    return '';
  }
  ...
  function goSoftPadFlowNode(nodeId) {
    ...
    if (nodeId === 'agent') {
      setSoftPadPadMode('skin', { fromUser: true });
      return;
    }
    if (nodeId === 'timeline') {
      return;
    ...
    if (nodeId === 'pad') {
      ...
      closeRingFloat();
      setSoftPadFace('pad', {
        padMode: normalizeSoftPadPadMode(lastSoftPadPadMode) === 'skin' ? 'keys' : (lastSoftPadPadMode || 'keys')
      });
    }
  }
  ...
  function handlePadRingAct(act) {
    ...
    if (act === 'show' || act === 'look') {
      setSoftPadFace('pad', { fromUser: true });
      setSoftPadPadMode(act === 'look' ? 'skin' : 'show', { fromUser: true });
      return;
    }
    if (act === 'enable') {
  ...
  function fourPanelSub(entry, id) {
    if (id === 'runtime') return t('softPadTileDisplaySub', '何时显示浮窗 + 换皮肤');
  ...
  function defaultDetailView(opts) {
    opts = opts || {};
    // Soft Pad page lands on「键位」— highest-frequency edit path.
  ...
  function syncSettingsPreviewBanner(face) {
    // Banner retired — keep data-preview-label on hosts for a11y only.
    void face;
    var previewLbl = t('softPadPreviewHostLbl', 'Soft Pad 预览');
  ...
  /** Face-mutex chrome: pad workspace (merged) or timeline. Agent face root stays hidden. */
  function syncFaceChrome(entry) {
    var e = els();
    var face = softPadFace === 'timeline' ? 'timeline' : 'pad';
    softPadFace = face;
    syncSettingsPreviewBanner(face);
    if (e.stage) {
      e.stage.setAttribute('data-soft-pad-face', face);
      e.stage.setAttribute('data-soft-pad-mode', softPadPadMode);
    ...
    if (panel) {
      panel.setAttribute('data-soft-pad-face', face);
      panel.setAttribute('data-soft-pad-mode', softPadPadMode);
    }
    ...
    showFace(e.faceAgent, false);
    if (e.faceAgent) {
      e.faceAgent.setAttribute('aria-hidden', 'true');
    }
    ...
    if (global.__otSoftPadEmptyIdleMounted && typeof global.__otSoftPadEmptyIdleSync === 'function') {
      ...
      if (idleTitle) {
        idleTitle.textContent = t('softPadDetailIdleTitle', '点中间键盘上的键');
      }
      if (idleSub) {
        idleSub.textContent = t('softPadDetailIdleSub', '选中后在这里告诉它做什么');
      }
    }
    syncSoftPadFlowNodes(entry);
    ensureSoftPadThreeColLayout();
    syncSoftPadRailChrome();
    if (e.pageBody) {
      e.pageBody.classList.toggle('is-face-agent', isAgentWorkbenchMode());
    }
    if (e.switcher) {
    ...
    var cursorCard = document.getElementById('cursorActivityMaintenanceCard');
    if (cursorCard) refreshCursorActivityPrefDom(cursorCard);
  ...
  function applySoftPadDetailChromeHost(model) {
    ...
    if (e.subHost) {
      ...
      // Hub syncSoftPadNeedAgentChrome owns subHost.hidden (改按钮 vs 样子/更多).
    }
  ...
  function syncRuntimeCheckboxes(entry) {
    ...
    body.querySelectorAll('[data-feature="occupy"]').forEach(function (card) {
    ...
    try {
      ...
      if (PadUi && typeof PadUi.syncNumpadReplaceChrome === 'function') {
        PadUi.syncNumpadReplaceChrome(body, pad);
        var dock = document.getElementById('softPadFloatDock');
        if (dock) PadUi.syncNumpadReplaceChrome(dock, pad);
      } else {
        var mode = pad.requireNumLockOff && pad.enabled ? 'occupy' : 'soft';
        body.querySelectorAll('[data-act="numpadMode"]').forEach(function (btn) {
    ...
    if (numpadMap) {
      numpadMap.setAttribute('data-numpad-on', (pad.requireNumLockOff && pad.enabled) ? '1' : '0');
  ...
  function onSoftPadPanelChanged(m, panel, changeOpts) {
    ...
    if (panel === 'layout') {
      updateStatusBar(entry);
      patchSchemeRowPresentation(entry);
      if (onPad && normalizeSoftPadPadMode(softPadPadMode) === 'keys') {
        softPadPreviewPaintForFloatTab(entry);
      }
      if (onPad || isAgentWorkbenchMode()) renderFuncTiles(entry);
      refreshSoftPadSceneKeys(entry);
    ...
    if (panel === 'agent') {
      updateStatusBar(entry);
      patchSchemeRowPresentation(entry);
      if (changeOpts.refreshPreview === false) {
        var PadMore = global.OneToneCodexMicroPadUi;
        if (PadMore && typeof PadMore.liveSoftPadFloatSkinLights === 'function') {
          try { PadMore.liveSoftPadFloatSkinLights(entry.mapping); } catch (_) {}
        }
        return;
      }
      softPadPreviewPaintForFloatTab(entry);
    ...
    if (panel === 'float-dock') {
      updateStatusBar(entry);
      patchSchemeRowPresentation(entry);
      // 盘边色/透明度滑块：只刷 CSS，禁止 remount（否则左侧 Soft Pad 被绑键预览冲掉）
      if (changeOpts.refreshPreview === false) {
        var PadLive = global.OneToneCodexMicroPadUi;
        if (PadLive && typeof PadLive.liveSoftPadFloatSkinLights === 'function') {
          try { PadLive.liveSoftPadFloatSkinLights(entry.mapping); } catch (_) {}
        }
        return;
      }
      softPadPreviewPaintForFloatTab(entry);
    ...
    updateStatusBar(entry);
    if (onPad && softPadPadMode === 'keys') softPadPreviewPaintForFloatTab(entry);
    if (onPad || isAgentWorkbenchMode()) renderFuncTiles(entry);
  }

  /** 何时出现 / 皮肤：别把左侧 appear 动画刷回绑键预览 */
  function softPadPreviewPaintForFloatTab(entry) {
    entry = entry || resolveSoftPadEntry();
    if (!hasMapping(entry)) return;
    var PadUi = global.OneToneCodexMicroPadUi;
    var tab = '';
    try {
      if (PadUi && typeof PadUi.getSoftPadFloatTab === 'function') {
        tab = String(PadUi.getSoftPadFloatTab() || '');
      }
    } catch (_) {}
    if (tab === 'show') {
      // 取消排队中的绑键预览，避免 setTimeout(0) 冲掉场景动画
      if (previewTimer) {
      ...
      ++previewEpoch;
      try {
        if (PadUi && typeof PadUi.refreshSoftPadFloatShowPreview === 'function') {
          PadUi.refreshSoftPadFloatShowPreview(entry.mapping);
        } else if (PadUi && typeof PadUi.paintSoftPadPadModePreview === 'function') {
          PadUi.paintSoftPadPadModePreview(previewHostForFace(), entry.mapping, 'appear');
        }
      } catch (_) {}
      return;
    }
    if (tab === 'skin') {
      ...
      ++previewEpoch;
      try {
        if (PadUi && typeof PadUi.refreshSoftPadFloatSkinPreview === 'function') {
          PadUi.refreshSoftPadFloatSkinPreview(entry.mapping);
        } else {
  ...
  function paintSubpage(entry, paintOpts) {
    ...
    if (softPadRailChannel && softPadRailChannel !== 'softPad') {
      ...
      if (typeof Pad.renderSoftPadChannelWorkbench === 'function' && channelHost) {
        try {
          ...
          try {
            if (typeof Pad.getSoftPadLayoutFocusKeyId === 'function') {
              keepKeyCh = String(Pad.getSoftPadLayoutFocusKeyId() || '').trim();
            }
          } catch (_) {}
          if (Pad.closeEditKeycap) Pad.closeEditKeycap({ reopenInline: false });
          ++subpageToken;
          Pad.renderSoftPadChannelWorkbench(channelHost, entry.mapping, softPadRailChannel, {
            onChanged: onChanged,
            focusKeyId: keepKeyCh
          });
        } catch (errCh) {
          feLog('fe softPad.paintSubpage channel ' + (errCh && errCh.message ? errCh.message : 'unknown'));
          throw errCh;
        } finally {
          feLog('fe softPad.paintSubpage channel:' + softPadRailChannel + ' ' + (Date.now() - t0) + 'ms');
        }
        return;
      }
    }
    ...
    try {
      ...
      if (targetView === 'layout' && Pad.renderSoftPadLayoutPanel) {
        ...
        var lookTab = normalizeSoftPadPadMode(softPadPadMode) === 'skin' ? 'skin' : 'show';
        if (typeof Pad.setSoftPadStyleSubtab === 'function') Pad.setSoftPadStyleSubtab(lookTab);
        Pad.renderSoftPadStylePanel(paintHost, m, {
          onChanged: onChanged,
          styleSubtab: lookTab,
          omitSubtabs: true
        });
        ...
        Pad.renderSoftPadDisplayPanel(paintHost, m, { onChanged: onChanged });
        ...
        syncAgentWorkbenchTabForPadMode(softPadPadMode);
      ...
      } else if (targetView === 'agent' && Pad.renderSoftPadMorePanel) {
        Pad.renderSoftPadMorePanel(paintHost, m, { onChanged: onChanged });
      } else {
  ...
  function setSoftPadFace(face, opts) {
    ...
    if (face === 'pad') {
      ...
      syncAgentWorkbenchTabForPadMode(mode);
    }
    if (hasMapping(entry)) {
  ...
  function setSoftPadPadMode(mode, opts) {
    ...
    syncFaceChrome(entry);
    paintSubpage(entry, { forceRemount: true });
    // Style paint / deferred catalog rAF can re-open fn-swap — re-assert workbench chrome.
    syncSoftPadNeedAgentChrome(entry);
    ensureSoftPadPreview(entry);
  }
  ...
  function openSubpage(view, openOpts) {
    ...
    if (view === 'tray') {
      if (global.OneToneSettingsDrawer && global.OneToneSettingsDrawer.open) {
        global.OneToneSettingsDrawer.open(Object.assign({ panel: 'tray' }, openOpts));
      }
      return;
    }
  ...
  function buildSoftPadWorkflowModel() {
    ...
    var onAgentFace = isAgentWorkbenchMode();
    // App scope lives in status-bar bind dropdown (v15); hide horizontal chip rail.
  ...
  function buildSoftPadPreviewModel() {
    ...
    var modePreview = softPadFace === 'pad' && isSoftPadLookMode();
    var floatTab = softPadFloatTabNow();
    // 何时显示 / 皮肤：左侧由 mid-ability 自绘，岛 sync 再刷绑键会冲掉动画/盘边预览
    if (floatTab === 'show' || floatTab === 'skin' || modePreview) {
      // appear/purpose/skin own left column — island must not paint keyboard.
      skipPaint = true;
    } else if (has && !force) {
      if (!canPaint) skipPaint = true;
      else if (paintedMappingId === mappingId) {
        var host = previewHostForFace();
        var paintEl = host && (host.querySelector('[data-soft-pad-preview-paint]') || host);
        // Keys mode used to repaint on every island sync → remount shell + wipe catalog/stats below.
  ...
  function paintPreview(entry, opts) {
    ...
    if (Pad && Pad.renderSoftPadPreview) {
      ...
      try {
        ...
        if (isAgentWorkbenchMode() && Pad.syncStatusLightsPreviewChrome) {
  ...
  function ensureSoftPadPreview(entry) {
    if (!hasMapping(entry)) return;
    var floatTab = softPadFloatTabNow();
    if (softPadFace === 'pad' && (floatTab === 'show' || floatTab === 'skin')) {
      var PadFloat = global.OneToneCodexMicroPadUi;
      try {
        if (floatTab === 'skin' && PadFloat && PadFloat.refreshSoftPadFloatSkinPreview) {
          PadFloat.refreshSoftPadFloatSkinPreview(entry.mapping);
        } else if (floatTab === 'show' && PadFloat && PadFloat.refreshSoftPadFloatShowPreview) {
          PadFloat.refreshSoftPadFloatShowPreview(entry.mapping);
        }
      } catch (_) {}
      return;
    }
    if (softPadFace === 'pad' && normalizeSoftPadPadMode(softPadPadMode) === 'show') {
      ...
      if (PadMode && typeof PadMode.paintSoftPadPadModePreview === 'function' && modeHost) {
        try {
          if (typeof PadMode.refreshSoftPadFloatShowPreview === 'function') {
            PadMode.refreshSoftPadFloatShowPreview(entry.mapping);
          } else {
    ...
    if (softPadFace === 'pad' && normalizeSoftPadPadMode(softPadPadMode) === 'skin') {
      var PadSkin = global.OneToneCodexMicroPadUi;
      try {
        if (PadSkin && PadSkin.refreshSoftPadFloatSkinPreview) {
          PadSkin.refreshSoftPadFloatSkinPreview(entry.mapping);
          paintedMappingId = String(entry.mapping.id);
        }
      } catch (_) {}
      return;
    ...
    // lights/mini need force so Soft Pad remount + outer chrome sync after purpose leftovers.
    paintPreview(entry, (softPadFace === 'pad' && !isAgentWorkbenchMode()) ? undefined : { force: true });
  }

  /** Single cancelable preview queue (keys-mode refresh). Uses previewEpoch, not selectToken. */
  function softPadFloatTabNow() {
    try {
      var PadUi = global.OneToneCodexMicroPadUi;
      if (PadUi && typeof PadUi.getSoftPadFloatTab === 'function') {
        return String(PadUi.getSoftPadFloatTab() || '');
      }
    } catch (_) {}
    return '';
  }
  ...
  function schedulePreviewPaint(entry) {
    ...
    // 何时出现 / 皮肤：左侧是场景或盘边预览，别用绑键 force 刷掉
    var floatTab = softPadFloatTabNow();
    if (floatTab === 'show' || floatTab === 'skin') return;
    ...
    previewTimer = setTimeout(function () {
      ...
      if (softPadFace !== 'pad' || softPadPadMode !== 'keys') return;
      var ft = softPadFloatTabNow();
      if (ft === 'show' || ft === 'skin') return;
      if (String(getSelectedMappingId() || '') !== mapId) return;
      var cur = findEntry(mapId);
      if (!hasMapping(cur)) return;
  ...
  function selectScheme(mappingId, opts) {
    ...
    if (opts.scopeId) {
      ...
      // FG follow must not lock the hub — only real user picks stick.
      if (!opts.fromForeground) scopePickedByUser = true;
    } else if (opts.fromList || !selectedScopeId) {
      selectedScopeId = entry.kind;
      if (opts.fromList && !opts.fromForeground) scopePickedByUser = true;
    }
    ...
    if (sameMap && hostsOk && !opts.forceRemount) {
      ...
      renderFuncTiles(entry);
      paintSubpage(entry, isAgentWorkbenchMode() ? { forceRemount: true } : undefined);
  ...
  function selectScope(scopeId, opts) {
    ...
    if (scope.mapping && scope.mapping.id) {
      selectScheme(String(scope.mapping.id), {
        fromList: true,
        fromForeground: !!opts.fromForeground,
        scopeId: selectedScopeId,
        rebuildList: !!opts.rebuildList,
        forceRemount: !!opts.forceRemount || isAgentWorkbenchMode(),
        resetView: opts.resetView === true,
        previewOnly: false
      });
      syncHabitEditFromSoftPadEntry(scope.entry || entryForScopeKind(scope.id));
      renderAppSwitcher();
      updateScopeHint();
      syncSoftPadPadRing();
      if (isAgentWorkbenchMode()) {
    ...
    requestOverlayUsageForScope(selectedScopeId);
    if (global.OneToneHabitChannelEditBanner && global.OneToneHabitChannelEditBanner.renderAll) {
  ...
  function renderSchemeList() {
    ...
    var fgBar = isAgentWorkbenchMode() ? renderForegroundAppBarHtml() : '';
    if (!entries.length) {
  ...
  function applyEnabledUi(entry) {
    ...
    if (!patchAppSwitcher()) renderAppSwitcher();
    updateScopeHint();
    if (softPadFace === 'pad' && isSoftPadLookMode()) {
      syncRuntimeCheckboxes(entry);
    }
  ...
  function toggleSelectedEnable() {
    var entry = statusBarEntry();
    if (!entry || !entry.mapping) return;
    if (!entry.mapping.codexMicroPad) {
      ensureSoftPadConfig(entry, { persist: true });
      applyRuntimeFlagsToEntry(entry);
    }
    if (!entry.mapping.codexMicroPad) return;
  ...
  function isAgentPanelCurrent(token, mappingId) {
    if (!isAgentWorkbenchMode()) return false;
  ...
  function bindChrome() {
    ...
    if (e.channelNav && !e.channelNav.__otRailBound) {
      e.channelNav.__otRailBound = true;
      e.channelNav.addEventListener('click', function (ev) {
        var tab = ev.target.closest && ev.target.closest('[data-soft-pad-channel]');
        if (!tab || !e.channelNav.contains(tab)) return;
        ev.preventDefault();
        setSoftPadRailChannel(tab.getAttribute('data-soft-pad-channel'), { fromUser: true });
      });
    }
    var restoreDefaults = document.getElementById('btnSoftPadRestoreDefaults');
    if (restoreDefaults && !restoreDefaults.__otRestoreBound) {
      restoreDefaults.__otRestoreBound = true;
      restoreDefaults.addEventListener('click', function (ev) {
        ev.preventDefault();
        var entry = resolveSoftPadEntry();
        if (!hasMapping(entry)) {
          toastPad(t('softPadPreviewEmptyNoMapping', '还没有可预览的虚拟键盘'));
          return;
        }
        var Pad = global.OneToneCodexMicroPadUi;
        if (Pad && typeof Pad.confirmRestoreSoftPadLayout === 'function') {
          Pad.confirmRestoreSoftPadLayout(entry.mapping);
          return;
        }
        toastPad(t('softPadPrepareFail', '准备失败：未能创建应用场景'));
      });
    }
    var bridgeGo = document.getElementById('softPadChannelBridgeGo');
    if (bridgeGo && !bridgeGo.__otBridgeBound) {
      bridgeGo.__otBridgeBound = true;
      bridgeGo.addEventListener('click', function () {
        openSoftPadChannelInKeys(softPadRailChannel);
      });
    }
    function onPreviewHintClick(ev) {
      var bindOpt = ev.target.closest && ev.target.closest('#softPadBindAppMenu [data-scope]');
      if (bindOpt) {
        ev.preventDefault();
        ev.stopPropagation();
        selectScope(bindOpt.getAttribute('data-scope'), { fromUser: true, resetView: false, forceRemount: isAgentWorkbenchMode() });
        return;
      }
    ...
    if (e.previewHint && !e.previewHint.__otHintBound) {
      e.previewHint.__otHintBound = true;
      e.previewHint.addEventListener('click', onPreviewHintClick);
    }
    if (e.tiles) {
    ...
    if (e.list) {
      e.list.addEventListener('click', function (ev) {
        ...
        selectScheme(btn.getAttribute('data-scheme-select'), {
          ...
          forceRemount: isAgentWorkbenchMode(),
          resetView: !isAgentWorkbenchMode()
        });
      });
    }
  ...
  function refreshSelected(m) {
    if (paintReentry > 0) return;
    var entry = syncEntryFromMapping(m);
    // Soft Pad mapping on + no occupy → physical 7 is still Home; turn occupy on quietly.
    try {
      var PadOcc = global.OneToneCodexMicroPadUi;
      if (PadOcc && PadOcc.ensurePhysicalNumpadOccupy && entry && entry.mapping) {
        PadOcc.ensurePhysicalNumpadOccupy(entry.mapping, { quiet: true, respectUserOff: true });
      }
    } catch (_) {}
    markActiveRow(getSelectedMappingId());
    if (!patchAppSwitcher()) renderAppSwitcher();
    ...
    if (entry) {
      ...
      if (softPadFace === 'pad' && softPadPadMode === 'keys') {
        var ftSel = softPadFloatTabNow();
        if (ftSel !== 'show' && ftSel !== 'skin') schedulePreviewPaint(entry);
      }
      syncFaceChrome(entry);
      if (softPadFace === 'pad' && isSoftPadLookMode()) {
        syncRuntimeCheckboxes(entry);
  ...
  function render(opts) {
    ...
    // Open Soft Pad: start FG follow immediately. Need-agent used to skip this and stick on「通用」.
    if (!opts.scopeId) {
      try { ensureForegroundFollow(); } catch (_) {}
    }
    var e = els();
    ...
    if (opts.mappingId) {
      ...
      // Prefer FG / existing pad over the module default "codex".
      var softFirst = resolveSoftPadEntry();
      if (hasMapping(softFirst)) {
        setSelectedMappingId(String(softFirst.mapping.id));
        if (!opts.scopeId) selectedScopeId = softFirst.kind;
      } else {
    ...
    if (!softPadAgentReady(softEntry)) {
      ...
      // Need-agent used to return before FG follow — Soft Pad stayed on「通用」until manual pick.
      ensureForegroundFollow();
      feLog('fe softPad.render need-agent ' + (Date.now() - t0) + 'ms scope=' + String(selectedScopeId || ''));
      return;
    }
    ...
    // Default landing: keys tab (highest-frequency edit path).
    var landMode = 'keys';
  ...
  function pruneSoftPadPanelChrome() {
    ['softPadBoundaryHint', 'habitChannelStatusStripSoftPad', 'softPadCursorActivityCard'].forEach(function (id) {
  ...
  function ensureCursorActivityConsentCard() {
    var host = document.getElementById('cursorActivityMaintenanceHost');
    if (!host) return;
    var card = document.getElementById('cursorActivityMaintenanceCard');
    if (!card) {
      card = document.createElement('div');
      card.id = 'cursorActivityMaintenanceCard';
      card.setAttribute('data-cursor-activity-card', '1');
      card.className = 'codex-pad-mgr__claude-act soft-pad-cursor-activity cursor-activity-maintenance-card';
      card.innerHTML =
        '<p class="codex-pad-mgr__label">' +
  ...
  function refreshCursorActivityPrefDom(card) {
    card = card || document.getElementById('cursorActivityMaintenanceCard');
    if (!card) return Promise.resolve();
  ...
  global.OneToneSoftPadHub = {
    ...
    setSoftPadRailChannel: setSoftPadRailChannel,
    openSoftPadChannelInKeys: openSoftPadChannelInKeys,
    ...
    getSelectedScopeId: function () {
      // Prefer the Soft Pad entry actually on screen — module default "codex" used to
      // stick in the status chrome while the pad preview already showed Cursor.
      try {
        var live = resolveSoftPadEntry();
        if (live && live.kind) return live.kind;
      } catch (_) {}
    ...
    isSoftPadKeysWorkbenchOpen: isSoftPadKeysWorkbenchOpen,
    isSoftPadLookMode: isSoftPadLookMode,
    applyScreenOpacityToPreview: applyScreenOpacityToPreview,
    ...
    canToggleSoftPadStatus: canToggleSoftPadStatus,
